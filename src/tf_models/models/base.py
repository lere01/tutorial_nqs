...
class Sampler(nn.Module):
    ...
    @torch.jit.export
    def sample(self, B: int, L: int) -> Tensor:
    ...
    @torch.jit.export
    def off_diag_labels_summed(self, sample: Tensor, nloops: int = 1) -> Tuple[Tensor, Tensor]:
        """
        Inputs:
            samples  - [B,L,1] matrix of zeros and ones for ground/excited states
...
@torch.jit.script
def genpatch2onehot(patch: Tensor, p: int) -> Tensor:
